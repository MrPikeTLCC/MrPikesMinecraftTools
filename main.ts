    ...
    // Tunnel Building
    function tunnelForward(distance: number, width: number, height: number) {
    ...
    player.onChat("tunnel", function (distance, width, height) {
        ...
        torchfrequency = 5
        agent.setSlot(1)
        tunnelForward(distance,width,height)
        agent.setAssist(DESTROY_OBSTACLES, false)
    })
    ...
    player.onChat("zigzagupright", function (segmentCount, size, height) {
        agent.setAssist(DESTROY_OBSTACLES, true)
        tunnelUp(size, size, height)
        tunnelForward(size, size, height)
        agent.turn(RIGHT)
        /*
        tunnelForward(size, size, height)
        tunnelUp(size, size, height)
        tunnelForward(size + 1, size, height)
        agent.move(BACK, 1)
        agent.place(FORWARD)
        agent.turn(LEFT)
